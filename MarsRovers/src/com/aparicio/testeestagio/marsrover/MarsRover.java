...
public class MarsRover {
	...
	public List<RoverCommand> transform(String input) {
		...
		for (int i = 1; i < inputLines.length; i += 2) {
			...
			RoverPosition position = new RoverPosition(Integer.valueOf(inputTokens[0]), Integer.valueOf(inputTokens[1]),
					Direction.valueOf(inputTokens[2]));
			command.setPosition(position);
			inputTokens = inputLines[i + 1].split("");
			List<Instruction> instructions = new ArrayList<Instruction>(inputTokens.length);
